...
app.post('/extract-isf', async (req, res) => {
  ...
  try {
    const message = await anthropic.messages.create({ model: 'claude-sonnet-4-5', max_tokens: 1000, system, messages: [{ role: 'user', content: userContent }] });
...
app.post('/file-isf', async (req, res) => {
  ...
  const sendEvent = (type, message, data = {}) => res.write(`data: ${JSON.stringify({ type, message, ...data })}\n\n`);
  try {
...
app.listen(PORT, () => console.log(`Film Logic ISF backend running on port ${PORT}`));
